...
describe('pull request cache layers', () => {
  const prefix = layerRestoreKey('linux', 'x64', 'v1-dir', 'rust-0123456789ab')

  it('keys a layer by the baseline bundle it was cut against', () => {
    expect(prefix).toBe('linux-x64-mbx-v1-dir-layer-rust-0123456789ab-')
  ...
  it('reads no baseline from a key it did not write', () => {
    expect(layerBaseline('linux-x64-mbx-v1-dir-rust-0123456789ab-abc', prefix)).toBeUndefined()
