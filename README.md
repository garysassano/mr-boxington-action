...
With `github-cache-mode: objects` and mbx 1.12.0 or later, a saving pull request stores a layer instead of a complete closure. Every closure a pull request exports repeats the default branch's baseline almost in full, and saving one per revision stores and uploads that baseline again each time. Instead, the pull request restores the baseline and its own latest layer side by side, lays the layer over the baseline, and imports the two as one bundle. After the job it exports the closure as before, then leaves out every object the baseline already holds, so each revision saves only what the pull request itself produced. A layer's key names the exact baseline bundle it was cut against (`<platform>-<architecture>-mbx-<generation>-layer-<toolchain>-<baseline>-run-<id>-<attempt>`). A new commit on the base branch does not orphan it, but once the base branch saves a new baseline, the layer no longer matches and the next revision starts again from the new baseline. A layer that still fails to import is dropped in favor of the baseline alone. The `target` payload, and an explicit `cache-key` or `restore-keys`, keep saving complete entries.
